...
public abstract class PrivacyProxy extends Thread {
	...
	public PrivacyProxy(Socket socket, Boolean autoFlush) {
		...
		try {
			...
			log("Something went wrong while setting socket timeout: "
					+ e.getMessage());
		}
	...
	public void run() {
			...
			if (cnt == 0) {
				...
				if (!dropped) {
					try {
						...
						firstLine = firstLineParts[0] + " " + firstLineParts[1]
								+ " " + firstLineParts[2];
						altered = firstLine + "\r\n";
			...
			if (!dropped) {
				...
				while (inOctets != -1 && !proxyDone) { // TODO rewrite to do-while?
						...
						if (responseHeaders.containsKey("Content-Length")) {
							try {
								contentLength = Integer
										.parseInt(responseHeaders
												.get("Content-Length"));
							} catch (NumberFormatException e) {
								log("Could not parse Content-Length header: "
										+ responseHeaders.get("Content-Length"));
							}
						} else if (responseHeaders
								.containsKey("Transfer-Encoding")) {
							chunked = responseHeaders.get("Transfer-Encoding")
									.equals("chunked");
						}
					...
					try {
						...
						log("Could not read anymore from the webserver, reason: "
								+ e.getMessage());
						proxyDone = true;
						toClient.flush();
						break;
		...
		if (!autoFlush) {
			System.out.println(String.format("Thread %d:%n%s", Thread
					.currentThread().getId(), logOutput.toString()));
		}
	}
}
