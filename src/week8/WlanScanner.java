...
import week8.Location.AverageWeightedLocationFinder;
...
public class WlanScanner {
	...
	// The ip address of the machine the Wlan scanner server is running on.
	// (Default = 127.0.0.1 , your own machine)
	...
	// Controls if the position is send to the server for viewing on the
	// website.
	public static boolean sendToWebsite = true;

	// The LocationFinder that is to be used. Replace this with your own
	// implementation!
	public LocationFinder locationHandler = new AverageWeightedLocationFinder();
	...
	/**
	 * Starts a thread for communications with the data server Starts a thread
	 * for displaying the location on a map Handles data passing between the
	 * 'communications thread' -> 'locationFinder' -> 'display thread'
	 */
	public WlanScanner() {
		data = new LinkedBlockingQueue<MacRssiPair[]>(); // Queue between
															// communicator
															// thread and this
															// thread
		pos = new LinkedBlockingQueue<Position>(); // Queue between this thread
													// and the viewer thread
		sendPos = new LinkedBlockingQueue<Position>(); // Queue between this
														// thread and the
														// WebSender thread
		...
		if (sendToWebsite) {
			Thread sender = new Thread(new WebSender(webSenderTarget, sendPos,
					name));
			sender.start();
		}
		...
		while (true) {
			...
			try {
				...
				if (sendToWebsite)
					sendPos.add(tempPos);
				System.out.println(tempPos); // When data is available give to
												// the locationFinder
				System.out.println("-----------------");
